    ...
    export default{
        data(){
            return {
                ...
                busy:true,//是否可以请求
                rules: {//校验规则
                username: [{required: true, message: '专业不能为空', trigger: 'blur'}],
                password: [{required: true, message: '用户不能为空', trigger: 'blur'}],
                },
                info: [],
                multipleSelection:[]
            }
        ...
        methods: {
            ...
            handleAdd() {
                // 发送Ajax请求
                this.$axios.post(`/api/user/tuser/`,{
            ...
            handleDelete(row) {
                ...
                this.$confirm("此操作将永久删除当前数据，是否继续？", "提示", {
                    ...
                    console.log(row)
                    this.$axios.delete(`/api/user/tuser/`,{data:{"tid":row.tid}}).then((res) => {
                        if (res.data.code === 200) {
            ...
            //查询全部用户信息
            getUserInfo(){
                this.$axios.get(`/api/user/tuser/`,).then(res=>{
                    if (res.data.code === 200) {
