...
<script>
    export default{
        ...
        methods: {
            ...
            handleAdd() {
                // 发送Ajax请求
                this.$axios.post(`/api/user/auser/`,{
                    id:this.FormData.id,
            ...
            handleDelete(row) {
                ...
                this.$confirm("此操作将永久删除当前数据，是否继续？", "提示", {
                    ...
                    console.log(row)
                    this.$axios.delete(`/api/user/auser/`,{data:{"id":row.id}}).then((res) => {
                        if (res.data.code === 200) {
            ...
            //查询全部班级信息
            getUserInfo(){
                this.$axios.get(`/api/user/auser/`,).then(res=>{
                    if (res.data.code === 200) {
